...
howtoHtml = do
  ...
  H.h1 $ H.a ! A.name "howto" $ "How to use npaste.de"
  ...
  H.ul $ do
    ...
    H.li $ do H.pre ! A.class_ "inline" $ "npaste.de/v"
              H.em "view"
              " all recent pastes"
    H.li $ do H.pre ! A.class_ "inline" $ "npaste.de/v/t/<TAG>"
              " view all recent pastes which contain the given "
              H.em "tag"
...
contactHtml = do
  ...
  H.h1 $ H.a ! A.name "contact" $ "Contact"
...
statisticsHtml :: Integer -> Integer -> Html
statisticsHtml numU numP = do

  H.h1 $ H.a ! A.name "statistics" $ "Statistics"
...
-- disclaimerHtml :: Html
-- disclaimerHtml = do -- TODO?

  -- H.h1 $ H.a ! A.name "disclaimer" $ "Disclaimer"
